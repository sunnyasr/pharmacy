...
public class DeliveredFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mOrderModelArrayList = new ArrayList<>();
        adapter = new MasterOrderAdapter(getContext(), mOrderModelArrayList);
        tvEmpty = (TextView) getActivity().findViewById(R.id.tv_empty);

        refreshLayout = (SwipeRefreshLayout) getActivity().findViewById(R.id.swipeRefreshLayout);
    ...
    private void getOrders() {
        ...
        kProgressHUD.show();
        StringRequest stringRequest = new StringRequest(Request.Method.GET, APIConfig.GET_ORDER_DELIVERED + userManager.getMID(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                kProgressHUD.dismiss();
                refreshLayout.setRefreshing(false);
                mOrderModelArrayList.clear();
                    ...
                    if (jsonArray.length() == 0) {
                        ...
                        if (jsonArray.length() > 0) {
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                mOrderModelArrayList.add(model);
